...
import {
  ArrowLeft, Star, ShoppingCart, ShieldCheck, Flame,
  Plus, Minus, Check, ChevronLeft, ChevronRight, XCircle
} from 'lucide-react';

export default function ProductDetail({ product, setView, onAddToCart }) {

  // ── GUARD TEMPRANO ──
  // Si el producto aún no está disponible, devolvemos un loader en lugar de null
  // para evitar la pantalla en blanco total mientras App.jsx resuelve el estado.
  if (!product) {
    return (
      <div className="min-h-screen bg-[#030308] flex items-center justify-center">
        <div className="w-12 h-12 border-4 border-phoenix-gold border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }

  // ── 1. PROCESAMIENTO SEGURO DE SABORES Y STOCK (JSONB) ──
  // Soporta tres formatos que puede devolver Supabase:
  //   a) Array de objetos JSONB → [{ name, stock }]        (nuevo esquema)
  //   b) Array de strings       → ['Mango', 'Uva']         (esquema anterior)
  //   c) null / undefined       → fallback a un sabor genérico
  const parsedFlavors = (() => {
    const raw = product.flavors;
    if (!raw || !Array.isArray(raw) || raw.length === 0) {
      // Fallback: toma el campo legado 'flavor' o un texto genérico
      return [{ name: product.flavor || 'Edición Especial', stock: 10 }];
    }
    return raw.map(f =>
      typeof f === 'string'
        ? { name: f, stock: 10 }          // string → adaptamos al nuevo formato
        : { name: f.name ?? 'Sin nombre', stock: f.stock ?? 10 }  // JSONB ya correcto
    );
  })();

  // Autoseleccionar el primer sabor que tenga stock; si todos están agotados, el primero.
  ...
  const currentFlavorObj = parsedFlavors.find(f => f.name === selectedFlavor) ?? parsedFlavors[0];
  const isOutOfStock     = currentFlavorObj?.stock === 0;

  // ── 2. PROCESAMIENTO SEGURO DE IMÁGENES ──
  // Soporta string (URL única) y Array de strings (galería).
  const images = (() => {
    if (!product.image_url) return [];
    return Array.isArray(product.image_url)
  ...
  // Ajustar cantidad cuando el sabor cambia y su stock es menor
  useEffect(() => {
    if (!currentFlavorObj) return;
    if (currentFlavorObj.stock === 0) {
      setQuantity(1);
    } else if (quantity > currentFlavorObj.stock) {
      setQuantity(currentFlavorObj.stock);
    }
  }, [selectedFlavor]); // eslint-disable-line react-hooks/exhaustive-deps

  // Resetear índice de imagen al montar
  useEffect(() => {
    setCurrentIndex(0);
  }, [product.id]);
  ...
  const handleAdd = () => {
    ...
    onAddToCart({ ...product, flavor: selectedFlavor, quantity });
    setIsAdded(true);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="bg-[#07070a]/50 border border-white/5 rounded-4xl p-10 flex items-center justify-center relative h-125 md:h-150 shadow-2xl backdrop-blur-sm group overflow-hidden select-none">
            ...
            {images.length > 0 ? (
              ...
              >
                {/* Imágenes apiladas con transición Fade */}
                {images.map((img, idx) => (
                  <img
                    key={idx}
                    src={img}
              ...
              /* Fallback visual si no hay imágenes en Supabase */
          ...
          <div className="flex flex-col justify-center">
              ...
              <div className="flex text-phoenix-gold">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    size={16}
                    fill={i < Math.floor(product.rating ?? 4.9) ? 'currentColor' : 'none'}
                    className={i < Math.floor(product.rating ?? 4.9) ? '' : 'opacity-40'}
                  />
                ))}
              </div>
              <span className="text-white/40 text-xs font-bold tracking-widest">
                {product.rating ?? '4.9'} · 248 reseñas
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10 font-sans">
              {[
                ...
                <div
                  key={i}
                  className="bg-white/2 border border-white/5 rounded-2xl p-5 text-center transition-all duration-300 hover:border-phoenix-gold/30 hover:bg-white/3 hover:-translate-y-1"
                >
            ...
            {/* ── SELECTOR DE SABORES CON INVENTARIO ── */}
            <div className="mb-10 font-sans">
              <div className="flex items-center justify-between mb-4">
                ...
                {/* Urgencia de stock bajo */}
                {!isOutOfStock && currentFlavorObj?.stock > 0 && currentFlavorObj.stock <= 5 && (
                    ...
                    <button
                      ...
                      className={`px-6 py-3.5 rounded-xl text-xs font-bold transition-all duration-300 border relative overflow-hidden ${
                        isExhausted
                          ...
                          : isSelected
                            ? 'bg-phoenix-gold/10 border-phoenix-gold text-phoenix-gold shadow-[0_0_15px_rgba(212,175,55,0.2)]'
            ...
            <div className="flex gap-4 mb-8 font-sans">
              ...
              <div className={`flex items-center justify-between rounded-2xl px-2 w-32 shrink-0 border transition-colors ${
                isOutOfStock ? 'bg-white/2 border-white/5 opacity-50' : 'bg-white/5 border-white/10'
              }`}>
                <button
                ...
                className={`flex-1 py-4.5 rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all duration-500 shadow-xl ${
                  isOutOfStock
                    ...
                    : isAdded
                      ...
                      : 'bg-linear-to-r from-phoenix-gold via-phoenix-gold-light to-phoenix-gold bg-size-[200%_auto] hover:bg-right text-black hover:scale-[1.02] active:scale-[0.98] shadow-[0_5px_20px_rgba(212,175,55,0.2)] hover:shadow-[0_8px_30px_rgba(212,175,55,0.4)]'
                }`}
              >
                {isOutOfStock ? (
            ...
            <div className="flex items-center gap-5 text-white/40 bg-white/2 border border-white/5 rounded-2xl p-6 transition-colors hover:border-phoenix-gold/20 font-sans">
              ...
              <p className="text-xs font-light leading-relaxed">
                <strong className="text-white font-bold mr-1">Garantía Fénix Elite:</strong> Producto 100% original con código QR de verificación. Si presenta fallas de fábrica en las primeras 48 horas, te lo cambiamos inmediatamente sin costo adicional.
              </p>
            </div>
...
ProductDetail.propTypes = {
  product: PropTypes.shape({
    ...
    flavor:       PropTypes.string,                  // campo legado (compatibilidad)
    flavors:      PropTypes.array,                   // array de strings o JSONB [{name, stock}]
    badge:        PropTypes.string,
    rating:       PropTypes.number,
    price:        PropTypes.number.isRequired,
    description:  PropTypes.string,
    puffs:        PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    nicotine:     PropTypes.string,
    rechargeable: PropTypes.string,
    color:        PropTypes.string,
    image_url:    PropTypes.oneOfType([             // string único o array de URLs
      PropTypes.string,
