...
export default function ProductCard({ product, onAddToCart, onViewProduct, isFavorite, onToggleFavorite }) {
  ...
  // ── 1. EXTRAER LA IMAGEN PRINCIPAL (ARRAY O STRING) ──
  const mainImage = Array.isArray(product.image_url)
    ? product.image_url[0]
    : product.image_url;

  // ── 2. CÁLCULO DE STOCK TOTAL (COMPATIBLE CON JSONB Y DATOS LEGADOS) ──
  // Soporta: array de objetos JSONB [{name, stock}], array de strings, o null.
  ...
  // ── 3. TEXTO DE SABORES DISPONIBLES ──
  // Muestra cuántos sabores tienen stock en lugar del campo legado 'flavor'.
  const availableFlavorsCount = parsedFlavors.filter(f => (f.stock ?? 0) > 0).length;
  const flavorsLabel = isOutOfStock
    ? 'Sin stock disponible'
    : availableFlavorsCount === 1
      ? `${parsedFlavors.find(f => f.stock > 0)?.name ?? '1 sabor'}`
      : `${availableFlavorsCount} sabores disponibles`;
  ...
  return (
    <div className={`group relative flex flex-col h-full overflow-hidden rounded-2xl border border-white/5 bg-white/2 p-6 transition-all duration-500 ${
      isOutOfStock
        ? 'opacity-80'
        : 'hover:bg-white/5 hover:border-phoenix-gold/30 hover:shadow-[0_0_30px_rgba(212,175,55,0.05)]'
    }`}>

      {/* ── BOTÓN DE FAVORITOS ── */}
      <button
        onClick={(e) => { e.stopPropagation(); if (onToggleFavorite) onToggleFavorite(product); }}
      ...
      {/* ── BADGE (AGOTADO O ETIQUETA PREMIUM) ── */}
      {isOutOfStock ? (
      ...
      >
        ...
        {mainImage ? (
          ...
          /* Fallback visual CSS si no hay imagen en Supabase */
      ...
      <div className="grow flex flex-col space-y-2">
        ...
        {/* ── CONTEO DE SABORES ──
            Reemplaza el campo legado 'product.flavor'.
            Si hay un solo sabor con stock, muestra su nombre directamente.
            Si hay varios, muestra "X sabores disponibles". */}
        <p className={`text-xs italic font-light ${isOutOfStock ? 'text-red-400/60' : 'text-white/40'}`}>
          {flavorsLabel}
        </p>
        ...
        <div className="mt-auto flex justify-between items-center pt-5 border-t border-white/5">
          ...
          <div className="flex gap-2">
            ...
            {/* Añadir al carrito
                CORRECCIÓN: hover:bg-phoenix-gold-light (clase canónica)
                en lugar de hover:bg-[#F3E5AB] (valor arbitrario) */}
            <button
              ...
              className={`flex items-center justify-center w-10 h-10 rounded-xl transition-all ${
                isOutOfStock
                  ...
                  : 'bg-phoenix-gold hover:bg-phoenix-gold-light text-black shadow-[0_0_15px_rgba(212,175,55,0.2)] hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] hover:scale-105'
              }`}
...
ProductCard.propTypes = {
  product: PropTypes.shape({
    ...
    flavor:   PropTypes.string,           // campo legado (compatibilidad con datos viejos)
    flavors:  PropTypes.oneOfType([       // nuevo esquema: array de strings o JSONB
